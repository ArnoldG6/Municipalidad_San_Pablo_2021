...
@WebServlet(name = "CommentServlet", urlPatterns = {
    "/API/CommentServlet/Retrieve/Comentarios",
    "/API/CommentServlet/Search"
})
public class CommentServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            switch (request.getServletPath()) {
                case "/API/CommentServlet/Retrieve/Comentarios":
                    retrieveComments(request, response);
                    break;
                case "/API/CommentServlet/Search":
     ...
     * a JSON formatted List<Comment> object, determined by a search in all 'Comment'
     * class attributes.
     */
...
//        responseJSON = new Gson().toJson(CommentDAO.getInstance().searchInAllColumns(requestJSON.getString("searchIncidence")));
...
//        }
     ...
     * with a JSON formatted List<Comment> object, determined by a sorting
     * criteria.
     */
    private void retrieveComments(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            response.setCharacterEncoding("UTF-8");
            requestJSON = new JSONObject(request.getReader().lines().collect(Collectors.joining()));
            responseJSON = new Gson().toJson(CommentDAO.getInstance().
