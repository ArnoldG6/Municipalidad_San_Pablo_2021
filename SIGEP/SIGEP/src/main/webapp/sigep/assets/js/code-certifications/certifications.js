function retrieveAsAdmin() {
    $.ajax({
        ...
        data: {},
        dataType: 'json',
        url: 'http://localhost:8086/home/BudgetCodeCertificateListService',
    ...
    var table = $('#certification-list').DataTable(
            {
                ...
                columns: [
                    ...
                    {title: "Documento", "render": function (data, type, row, meta) {
                            ...
                            return "<button type='button' id='blank' class='btn btn-outline-warning btn-sm ' value=" + r + " onclick=pdfWindowCCP(event)>Descargar <i class='fa fa-solid fa-file-arrow-down'></i></button>";
                        }
                    }
                ]
            });
...
function pdfWindowCCP(event) {
    let path = event.target.value;

    const result = path.split(".")[0].split("\\").slice(-1);

    document.getElementById("nombre-ccp").value = result;
    document.getElementById("form-ccp").submit();
